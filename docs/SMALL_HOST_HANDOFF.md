...
- 小主机源码已纳入 Git 管理。
- 前端入口已拆出壳层：`frontend/src/components/shell/` 存放导航配置、侧边栏、顶部栏、账号弹窗和 `useNavigationState.ts`；`frontend/src/styles/` 存放壳层和登录样式；`HumanApp.tsx` 只保留登录态、基础数据刷新、账号表单和页面分发。
...
5. 继续拆分视频生产旧大文件：优先处理 `app/api/v1/human_workflow.py`、`app/services/jianying_drafts.py`、`app/services/music_resources.py`；前端继续把 `HumanApp.tsx` 中的基础数据刷新和账号表单状态拆成 hook。
