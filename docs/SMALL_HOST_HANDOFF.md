...
禁止：不下载本地大模型；不把厂商 API Key 写进画布 JSON；不改视频生产导航
...
- AI 视频商品图由用户直接上传，并创建 `kind=product` 的 AI 视频资产。
...
6. 为 AI 视频补真实浏览器交互测试，覆盖项目新建/删除、商品图上传、任务提交和任务刷新。

## 9. 当前平台模块

本项目只继续维护视频生产和 AI 视频；模型配置作为后台支撑能力保留。图片生产已清除，店铺经营与日常运营数据统一进入抖店管理平台。
